...
pub const BETA: Fp = Fp::new_unchecked([
    0x30f1361b798a64e8,
    0xf3b8ddab7ece5a2a,
    0x16a8ca3ac61577f7,
    0xc26a2ff874fd029b,
...
const B3: Fp = B.add_const(B).add_const(B);
...
fn endomorphism(p: &VestaAffine) -> VestaAffine {
    ...
    // where BETA is a non-trivial cubic root of unity in Fp.
    let mut res = *p;
    res.x *= BETA;
    res
}
...
impl Mul<Fq> for VestaProjective {
    type Output = VestaProjective;
...
impl Mul<VestaProjective> for Fq {
    type Output = VestaProjective;
...
mod tests {
    ...
    curve_test!(bls12_381, Fq, VestaAffine, VestaProjective, 100);
