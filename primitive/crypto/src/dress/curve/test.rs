...
macro_rules! curve_test {
    ($test_name:ident, $field:ident, $affine:ident, $projective:ident, $iter_times:expr) => {
        ...
        mod tests {
            ...
            fn arb_projective<C: Projective>() -> C {
                C::GENERATOR * C::Scalar::random(OsRng)
            }

            fn arb_affine<C: Affine>() -> C {
                C::GENERATOR * C::Scalar::random(OsRng)
            }
            ...
            paste! {
                ...
                fn [< $test_name _affine_addition_test >]() {
                    for _ in 0..$iter_times {
                        let a = arb_affine::<$affine>();
                        let b = arb_affine::<$affine>();
                        let c = arb_affine::<$affine>();
            ...
            paste! {
                ...
                fn [< $test_name _projective_is_on_curve_test >]() {
                    assert!($projective::GENERATOR.is_on_curve());
            ...
            paste! {
                ...
                fn [< $test_name _projective_identity_test >]() {
                    let a = arb_projective::<$projective>();
                    // a + (-a) = e
                    let identity = a - a;
            ...
            paste! {
                ...
                fn [< $test_name _projective_addition_test >]() {
                    for _ in 0..$iter_times {
                        let a = arb_projective::<$projective>();
                        let b = arb_projective::<$projective>();
                        let c = arb_projective::<$projective>();
            ...
            paste! {
                ...
                fn [< $test_name _projective_scalar_test >]() {
                    for _ in 0..$iter_times {
                        let g = arb_projective::<$projective>();
