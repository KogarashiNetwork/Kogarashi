...
impl Circuit<JubjubAffine> for ConfidentialTransferCircuit {
    ...
    fn synthesize(&self, composer: &mut Groth16<JubjubAffine>) -> Result<(), Error> {
        ...
        let sender_private_key = composer.alloc_witness(self.sender_private_key.into());
        let transfer_amount = composer.alloc_witness(self.transfer_amount.into());
        let sender_after_balance = composer.alloc_witness(self.sender_after_balance.into());
        let randomness = composer.alloc_witness(self.randomness.into());
